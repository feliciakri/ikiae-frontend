...
export default function Order(props: any) {
  ...
  const isAmount = order.product_data
    ?.map((product: any) => Number(product.product_price))
    .reduce((a: any, b: any) => a + b);
  const totalAmount = formatRupiah(isAmount);
  return (
    <div
      key={order.number}
        ...
        <dl className="flex-1 grid grid-cols-2 gap-x-6 text-sm sm:col-span-3 sm:grid-cols-3 lg:col-span-2">
          ...
          <div className="hidden sm:block">
            ...
            <dd className="mt-1 text-gray-500">
              <time dateTime={order.createdDatetime}>{order.order_date}</time>
            </dd>
          </div>
          <div>
            <dt className="font-medium text-gray-900">Total amount</dt>
            <dd className="mt-1 font-medium text-gray-900">{totalAmount}</dd>
          </div>
        </dl>
